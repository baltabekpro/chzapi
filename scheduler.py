...
class Scheduler:
    ...
    def run_test(self, delay_minutes=2):
        ...
        scheduler_logger.info(f"Test process scheduled for {target_time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        try:
            ...
            while datetime.now() < target_time:
                ...
                if remaining > 5:  # Only log if more than 5 seconds remain
                    scheduler_logger.info(f"Waiting for test process: {int(remaining)} seconds remaining")
                    time.sleep(min(remaining, 30))  # Sleep for up to 30 seconds at a time
                else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--test":
        ...
        scheduler_logger.info(f"Running scheduler in test mode with {delay_minutes} minute delay")
        scheduler = Scheduler()
