    ...
    for cert_dir in os.listdir(base_dir):
        ...
        if os.path.exists(report_file):
            try:
                ...
                all_reports.append({"certificate": cert_dir, "data": report_data})
    ...
    for report in all_reports:
        ...
        for product_group, count in report["data"].get("violations", {}).items():
            ...
            try:
                ...
                email_logger.debug(
                    f"Product: {product_group}, Count: {count_value}, Type: {type(count_value)}"
                )
            except (ValueError, TypeError) as e:
                email_logger.warning(
                    f"Invalid count value for {product_group}: {count} ({type(count)})"
                )
...
def send_regional_reports() -> bool:
    ...
    email_logger.info(
        f"Found {len(regional_reports)} regional reports: {', '.join(regional_reports.keys())}"
    )
    email_logger.info(
        f"Region data contains {len(regions_data)} regions: {', '.join(regions_data.keys())}"
    )
    ...
    for region, report_data in regional_reports.items():
        ...
        try:
            ...
            email_logger.info(
                f"Processing report for region: {region} (display name: {region_name})"
            )
            ...
            for product_group, count in sorted(
                report_data["violations"].items(), key=lambda x: x[1], reverse=True
            ):
                html += f"""
                <tr>
            ...
            msg[
                "Subject"
            ] = f"Отчет о нарушениях маркировки - Регион {region_name} - {report_data['date']}"
            msg["From"] = email_config["sender_email"]

            # Get recipients for this region
            recipients = regions_data.get(region, {}).get(
                "emails", email_config["recipient_emails"]
            )
            if not recipients:
            ...
            email_logger.info(
                f"Sending email for region {region_name} to {len(recipients)} recipients"
            )
            ...
            email_logger.info(
                f"Email report sent successfully for region {region_name}"
            )

        except Exception as e:
            log_exception(
                email_logger, e, f"Error sending email report for region {region}"
            )
            success = False
...
def process_and_send_reports() -> bool:
    ...
    try:
        ...
        if result:
            ...
            email_logger.warning(
                "Some issues occurred while processing and sending reports"
            )
