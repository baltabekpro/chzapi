...
def get_auth_data():
    ...
    for attempt in range(1, max_retries + 1):
        try:
            print(
                f"{Fore.CYAN}Попытка получения данных для аутентификации ({attempt}/{max_retries})..."
            )

            # Добавляем таймауты и настройки сессии
            session = requests.Session()
            session.headers.update(
                {"Accept": "application/json", "User-Agent": "CRPT-API-Client/1.0"}
            )
            ...
            print(
                f"{Fore.RED}❌ Ошибка подключения (попытка {attempt}/{max_retries}): {str(e)}"
            )
            ...
            if attempt < max_retries:
                ...
                print(
                    f"{Fore.CYAN}⏳ Ожидание {wait_time} секунд перед повторной попыткой..."
                )
            ...
            print(
                f"{Fore.RED}❌ Превышено время ожидания (попытка {attempt}/{max_retries}): {str(e)}"
            )
            if attempt < max_retries:
                wait_time = retry_delay * attempt
                print(
            ...
            print(
                f"{Fore.RED}❌ Неожиданная ошибка (попытка {attempt}/{max_retries}): {str(e)}"
            )
            if attempt < max_retries:
                wait_time = retry_delay * attempt
                print(
    ...
    print(
        f"{Fore.RED}❌ Не удалось получить данные для аутентификации после {max_retries} попыток"
    )
    ...
    print(
        f"{Fore.YELLOW}   3. Попробуйте позже (возможны временные проблемы на сервере)"
    )
...
def sign_data_with_cryptcp(data_file, thumbprint=None, pin=None):
    ...
    try:
        ...
        cmd = [
            ...
            "-der",  # Формат DER
            "-thumbprint",
            thumbprint,  # Сертификат по отпечатку
        ...
        if result.returncode == 0:
            ...
            if result.stdout:
                print(f"{Fore.RED}STDOUT: {result.stdout}")
            if result.stderr:
                print(f"{Fore.RED}STDERR: {result.stderr}")
            return None
...
def get_token(uuid, signed_data, inn=None, use_mchd=False):
    ...
    try:
        ...
        request_data = {"uuid": uuid, "data": signed_data}
        ...
        for attempt in range(1, max_retries + 1):
            try:
                print(
                    f"{Fore.CYAN}Попытка получения токена ({attempt}/{max_retries})..."
                )
                ...
                print(
                    f"{Fore.RED}❌ Ошибка подключения при получении токена (попытка {attempt}/{max_retries}): {str(e)}"
                )
                if attempt < max_retries:
                    wait_time = retry_delay * attempt
                    print(
                ...
                print(
                    f"{Fore.RED}❌ Превышено время ожидания при получении токена (попытка {attempt}/{max_retries})"
                )
                if attempt < max_retries:
                    wait_time = retry_delay * attempt
                    print(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"{Fore.YELLOW}Ответ сервера не содержит токен. Статус: {response.status_code}"
                )
                return None, "api_error"
...
def save_tokens_json(tokens):
    ...
    try:
        ...
        with open("true_api_tokens.json", "w", encoding="utf-8") as f:
            json.dump(
                {"tokens": existing_tokens, "generated_at": datetime.now().isoformat()},
                f,
                indent=2,
                ensure_ascii=False,
            )
            ...
            if tc_inn_pairs:
                ...
                for pair in tc_inn_pairs:
                    for tc, inn in pair.items():
                        ...
                        signature_path = sign_data_with_cryptcp(
                            "data_to_sign.txt", thumbprint
                        )
                        ...
                        if token and status == "success":
                            print(
                                f"{Fore.GREEN}Токен успешно получен для {name} - {tc}"
                            )
                ...
                if status == "require_inn":
                    tc = input(
                        f"{Fore.YELLOW}Введите ТС для сертификата {name}: "
                    ).strip()
