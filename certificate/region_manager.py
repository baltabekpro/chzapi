...
def manage_regions():
    ...
    while True:
        ...
        if choice == "0":
            ...
            emails_str = input(
                f"{Fore.YELLOW}Enter email addresses (comma separated): "
            )
            ...
            for i, (region_id, region_data) in enumerate(regions_data.items(), 1):
                ...
                if tc_list:
                    print(
                        f"{i}. {region_data.get('name', region_id)} ({region_id}) - {len(tc_list)} TCs"
                    )
            ...
            try:
                ...
                confirm = input(
                    f"{Fore.RED}Are you sure you want to delete {regions_data[region_id].get('name', region_id)}? (y/n): "
                )
                if confirm.lower() == "y":
