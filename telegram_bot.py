...
def acquire_lock():
    ...
    try:
        ...
        lock_socket = None
        telegram_logger.warning(f"Another bot instance is already running (port {LOCK_PORT} in use)")
        return False
...
class TelegramBot:
    ...
    def __init__(self):
        ...
        self.error_notification_chat_ids = self.config.get('error_notification_chat_ids', [])
        ...
        self.connection_timeout = self.config.get('connection_timeout', 60)  # Increased from 30 to 60 seconds
        self.cleanup_on_start = self.config.get('cleanup_on_start', True)  # Whether to clean up existing sessions
        ...
        # Error notification queue - we'll properly initialize it in the event loop
        self._pending_errors = []  # Thread-safe storage for error messages
        # self.error_queue will be initialized in error_monitoring_task
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        try:
            if os.path.exists('telegram_config.json'):
                ...
                telegram_logger.warning("Created default telegram_config.json. Please update with your bot token.")
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if user_id not in self.allowed_users and str(user_id) not in self.allowed_users:
            ...
            telegram_logger.warning(f"Unauthorized access attempt from {username} (ID: {user_id})")
            return
            
        # User is authorized
        admin_status = "Администратор" if user_id in self.admin_users or str(user_id) in self.admin_users else "Пользователь"
    ...
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        for command, description in self.command_registry.items():
            ...
            if command in ["restart", "run"] and (user_id not in self.admin_users and str(user_id) not in self.admin_users):
                continue
    ...
    async def status_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            try:
                ...
                telegram_logger.info(f"Scheduler status check: PID={scheduler_pid}, Status={scheduler_status}")
            except Exception as e:
            ...
            if last_run_info:
                ...
                run_type = "Ручной" if last_run_info.get('manual_run', False) else "Автоматический"
            ...
            token_status = f"🟢 Имеется {token_count} токенов" if token_count > 0 else "🔴 Токены не найдены"
            ...
            report_count = sum(len(certs) for certs in reports.values()) if reports else 0
    ...
    async def restart_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if user_id not in self.admin_users and str(user_id) not in self.admin_users:
            ...
            telegram_logger.warning(f"Unauthorized restart attempt from user ID: {user_id}")
            return
        ...
        try:
            ...
            if new_pid:
                await update.message.reply_text(f"✅ Планировщик успешно перезапущен. Новый PID: {new_pid}")
                telegram_logger.info(f"Scheduler restarted with PID {new_pid} by user ID: {user_id}")
            else:
                await update.message.reply_text("❌ Ошибка при перезапуске планировщика")
                telegram_logger.error(f"Failed to restart scheduler, requested by user ID: {user_id}")
                
        except Exception as e:
            log_exception(telegram_logger, e, "Error in restart command")
            await update.message.reply_text(f"❌ Ошибка при перезапуске планировщика: {str(e)}")
            
    async def reports_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            await update.message.reply_text("📊 Выберите сертификат для просмотра отчетов:", reply_markup=reply_markup)
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            # Handle certificate selection
                ...
                if cert_name in reports:
                    ...
                    for report in cert_reports:
                        ...
                        # Make sure path is shortened if needed to avoid callback_data length issues
                        report_path = report['path']
                        if len(f"report_{report_path}") > 60:  # Keep within safe limit for callback_data
                            # Create a shorter unique identifier
                            import hashlib
                            id_hash = hashlib.md5(report_path.encode()).hexdigest()[:10]
                            # Store the mapping for later retrieval
                            if not hasattr(self, '_report_path_map'):
                                self._report_path_map = {}
                            self._report_path_map[id_hash] = report_path
                            keyboard.append(
                                [InlineKeyboardButton(f"Отчет от {report_date}", callback_data=f"rep_{id_hash}")]
                            )
                        else:
                            keyboard.append(
                                [InlineKeyboardButton(f"Отчет от {report_date}", callback_data=f"report_{report_path}")]
                            )
                    
                    # Add back button
                    keyboard.append([InlineKeyboardButton("« Назад к сертификатам", callback_data="back_to_certs")])
            ...
            # Handle shortened report selection
                ...
                id_hash = callback_data[4:]  # Remove "rep_" prefix
                if hasattr(self, '_report_path_map') and id_hash in self._report_path_map:
                    report_path = self._report_path_map[id_hash]
                    ...
                    await query.edit_message_text(f"❌ Не удалось найти отчет")
            ...
            # Handle back button
                ...
                reply_markup = InlineKeyboardMarkup(keyboard)
                await query.edit_message_text("📊 Выберите сертификат для просмотра отчетов:", reply_markup=reply_markup)
        ...
        for filename in os.listdir(logs_dir):
            if filename.endswith('.log'):
                ...
                try:
                    ...
                    log_exception(telegram_logger, e, f"Error reading log file {filepath}")
        ...
        def run_process():
            try:
                ...
                if result:
                    ...
                    telegram_logger.info(f"Daily process completed successfully, requested by user ID: {user_id}")
                else:
                    loop.run_until_complete(
                        context.bot.send_message(
                            chat_id=update.effective_chat.id,
                    ...
                    telegram_logger.error(f"Daily process failed, requested by user ID: {user_id}")
                ...
                try:
                    ...
                    error_message = f"❌ Ошибка при выполнении ежедневной обработки: {str(e)}"
                    loop.run_until_complete(
                        context.bot.send_message(
                            chat_id=update.effective_chat.id,
                            text=error_message
    ...
    async def tokens_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if os.path.exists(token_file):
                ...
                # Fix: Use Latin 'M' instead of Cyrillic 'М' - correct format string
                message += f"\nПоследнее обновление: {mod_time.strftime('%Y-%m-%d %H:%M:%S')}"
                
            # Add option to refresh tokens - Use a shorter callback_data to avoid Button_data_invalid error
            keyboard = [[InlineKeyboardButton("🔄 Обновить токены", callback_data="refresh")]]
            reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if message_text.lower() in ["привет", "здравствуйте", "hi", "hello"]:
            await update.message.reply_text(f"Здравствуйте! Используйте /help для списка команд.")
    ...
    async def check_authorization(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        ...
        if user_id not in self.allowed_users and str(user_id) not in self.allowed_users:
            await update.message.reply_text("❌ У вас нет доступа к этому боту")
            telegram_logger.warning(f"Unauthorized access attempt from user ID: {user_id}")
            return False
    ...
    async def send_error_notification(self, error_message: str) -> None:
        ...
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # Fixed format string here
        ...
        for chat_id in self.error_notification_chat_ids:
            try:
                ...
                log_exception(telegram_logger, e, f"Failed to send error notification to chat ID: {chat_id}")
    ...
    async def error_monitoring_task(self) -> None:
        """Background task to process and send error notifications."""
        telegram_logger.info("Starting error monitoring task")
        self.error_queue = asyncio.Queue()  # Initialize the error queue in the event loop
        while self.running:
            try:
                # Get error from queue with timeout
                try:
                    # Use asyncio.get_running_loop() to ensure we're using the current event loop
                    error_message = await asyncio.wait_for(self.error_queue.get(), timeout=1.0)
                    ...
                    # No errors in queue, just continue
                    pass
                except RuntimeError as e:
                    if "different event loop" in str(e):
                        telegram_logger.error(f"Event loop error in monitoring task: {e}")
                        # Create a new queue bound to this event loop
    ...
    async def cleanup_telegram_session(self) -> None:
        ...
        try:
            ...
            telegram_logger.info(f"Cleanup response: {response.status_code}, {response.text}")
    ...
    async def start_bot(self) -> None:
        ...
        if not acquire_lock():
            telegram_logger.error("Cannot start bot: Another bot instance is already running")
            
            # Try a more aggressive cleanup approach
            try:
                telegram_logger.info("Attempting forced cleanup of any existing sessions...")
                import requests
                # Make a direct API call to delete webhook and reset getUpdates state 
                reset_url = f"https://api.telegram.org/bot{self.token}/deleteWebhook?drop_pending_updates=true"
                response = requests.get(reset_url, timeout=10)
                telegram_logger.info(f"Forced cleanup response: {response.status_code}, {response.text}")
                ...
                if not acquire_lock():
                    telegram_logger.error("Still cannot acquire lock. Exiting bot initialization.")
                    return
            except Exception as e:
        ...
        try:
            ...
            asyncio.create_task(self.error_monitoring_task())
            
            # Initialize the bot with retry mechanism
            for attempt in range(1, self.max_retries + 1):
                try:
                    telegram_logger.info(f"Starting Telegram bot (attempt {attempt}/{self.max_retries})")
                    
                    # Add a longer timeout for initialization
                    await asyncio.wait_for(
                        self.application.initialize(),
                        timeout=self.connection_timeout * 3  # Increased multiplier from 2 to 3
                    )
                    ...
                    while self.running:
                        ...
                        if heartbeat_counter >= 10:  # Log heartbeat every 5 minutes
                            telegram_logger.debug("Telegram bot heartbeat: still running")
                            heartbeat_counter = 0
                    ...
                    telegram_logger.error(f"Timeout initializing bot (attempt {attempt}/{self.max_retries})")
                    if attempt < self.max_retries:
                        ...
                        telegram_logger.error("Maximum retry attempts reached. Failed to start Telegram bot.")
                        self.running = False
                        break
                    ...
                    try:
                        ...
                        reset_url = f"https://api.telegram.org/bot{self.token}/deleteWebhook?drop_pending_updates=true"
                        response = requests.get(reset_url, timeout=10)
                        telegram_logger.info(f"Reset response: {response.status_code}, {response.text}")
                        
                        # Wait longer before retry for conflict errors
                        wait_time = self.retry_delay * 3
                        telegram_logger.info(f"Waiting {wait_time} seconds before retry...")
                        await asyncio.sleep(wait_time)
                        ...
                        telegram_logger.error(f"Error during connection reset: {cleanup_error}")
                        self.running = False
                        break
                        
                except telegram.error.TimedOut:
                    telegram_logger.error(f"Telegram API timed out (attempt {attempt}/{self.max_retries})")
                    if attempt < self.max_retries:
                        wait_time = self.retry_delay * (2 ** (attempt - 1))
                        telegram_logger.info(f"Retrying in {wait_time} seconds...")
                        await asyncio.sleep(wait_time)
                    ...
                    telegram_logger.error(f"Network error starting bot: {e} (attempt {attempt}/{self.max_retries})")
                    if attempt < self.max_retries:
                        wait_time = self.retry_delay * (2 ** (attempt - 1))
                        telegram_logger.info(f"Retrying in {wait_time} seconds...")
                        await asyncio.sleep(wait_time)
                    ...
                    log_exception(telegram_logger, e, f"Error starting Telegram bot (attempt {attempt}/{self.max_retries})")
                    self.running = False
                    break
    ...
    def queue_error(self, error_message: str) -> None:
        ...
        if not self.running or not self.initialized:
            telegram_logger.warning("Bot is not running or not initialized, error notification queued but won't be sent")
            return
            
        try:
            # If we're in the same thread as the bot's event loop
            if threading.current_thread() == self.thread and self.application and hasattr(self.application, "loop"):
                try:
                    # Use the application's event loop if available
                    ...
                    telegram_logger.warning(f"Could not use application loop: {e}")
                    
            # Direct approach for the main bot thread or if application loop not available
            try:
                if threading.current_thread() == self.thread:
                    # We're in the bot thread, get the current event loop
                    loop = asyncio.get_event_loop()
                    loop.create_task(self.error_queue.put(error_message))
                    telegram_logger.info(f"Error queued for notification using current event loop: {error_message}")
                    return
            except Exception as e:
                telegram_logger.warning(f"Could not use current event loop: {e}")
                
            # Fall back to simple notification storage for other threads
            telegram_logger.info(f"Storing error for notification (from external thread): {error_message}")
            
            # Store the error message for later notification
            # This is a thread-safe operation that won't use asyncio
            with threading.Lock():
