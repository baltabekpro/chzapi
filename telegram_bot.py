    ...
    def get_queue(self):
        ...
        with self._lock:
            if tid not in self._queues:
                self._queues[tid] = asyncio.Queue(maxsize=100)  # Limit size to prevent memory issues
...
class TelegramBot:
    ...
    async def tokens_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            keyboard = [[InlineKeyboardButton("🔄 Обновить токены", callback_data="refresh_tokens")]]
            reply_markup = InlineKeyboardMarkup(keyboard)
                ...
                try:
                    ...
                    if self.initialized and not self.error_queue.empty():
                        ...
                        if error:
                            ...
                            if self.application is not None and hasattr(self.application, 'bot'):
                                for chat_id in self.error_notification_chat_ids:
                                    self._send_message_sync(chat_id, error)
                            else:
                                telegram_logger.debug("Cannot send error - bot not fully initialized")
    ...
    async def send_error_notification(self, error: str) -> None:
        ...
        if not self.application or not self.initialized:
            telegram_logger.debug("Cannot send error notification - bot not initialized")
            return
        ...
        for chat_id in self.error_notification_chat_ids:
            try:
                ...
                telegram_logger.error(f"Failed to send error notification to {chat_id}: {e}")
