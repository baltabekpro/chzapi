...
def generate_report_for_region(region_id: str, region_info: dict) -> dict:
    """
    Generate a report for a specific region
    
    Args:
        region_id: The ID of the region
        region_info: Dictionary containing region configuration
        
    Returns:
        dict: Report data for the region, or None if no data
    """
    logger.info(f"Generating report for region: {region_id}")
    
    region_name = region_info.get('name', f'Регион {region_id}')
    tc_list = region_info.get('tc_list', [])
    ...
    if not os.path.exists(base_dir):
        logger.warning(f"Base output directory not found: {base_dir}")
        return None
    
    # Find all certificate directories
    cert_dirs = [d for d in os.listdir(base_dir) if os.path.isdir(os.path.join(base_dir, d))]
    if not cert_dirs:
        logger.warning("No certificate directories found")
        return None
    
    # Process reports for each certificate
    found_data = False
    for cert_name in cert_dirs:
        # Check if this certificate belongs to this region
        # If tc_list is empty, include all certificates
        if tc_list and cert_name not in tc_list:
            continue
            
        cert_dir = os.path.join(base_dir, cert_name)
        
        # Look for violation reports
        report_files = [f for f in os.listdir(cert_dir) 
                        if f.startswith('violations_') and f.endswith('.json')]
        
        for report_file in report_files:
            try:
                # Check if report is from yesterday
                report_date = report_file.split('_')[1].split('.')[0]
                if report_date != yesterday:
                    continue
                    
                # Read the report
                report_path = os.path.join(cert_dir, report_file)
                with open(report_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                
                # Aggregate violations by product group
                for group, count in data.get('violations', {}).items():
                    if group not in report['violations']:
                        report['violations'][group] = 0
                    report['violations'][group] += count
                    report['total_violations'] += count
                    
                found_data = True
                logger.info(f"Added report data from {cert_name}/{report_file}")
                
            except Exception as e:
                log_exception(logger, e, f"Error processing report {report_file} for {cert_name}")
    
    # Return report if we found any data
    if found_data:
        logger.info(f"Generated report for {region_name} with {report['total_violations']} total violations")
        return report
    else:
        logger.warning(f"No report data found for region {region_name}")
        return None

def process_reports():
    """Process reports for all regions"""
    logger.info("Starting report processing for all regions")
    ...
    # Track reports by region
    reports_by_region = {}
    
    # Process each region
    for region_id, region_info in regions.items():
        logger.info(f"Processing region: {region_id} ({region_info.get('name', 'Unnamed')})")
        
        # Process region data and generate report
        try:
            region_report = generate_report_for_region(region_id, region_info)
            if region_report:
                reports_by_region[region_id] = region_report
                logger.info(f"Successfully generated report for region {region_id}")
            else:
                logger.warning(f"No report data generated for region {region_id}")
        except Exception as e:
            log_exception(logger, e, f"Error processing region {region_id}")
    
    # Send reports via email if we have any
    if reports_by_region:
        try:
            from email_sender import send_report_emails
            send_success = send_report_emails(reports_by_region)
            if send_success:
                logger.info(f"Successfully sent reports for {len(reports_by_region)} regions")
            else:
                logger.error("Failed to send reports via email")
        except Exception as e:
            log_exception(logger, e, "Error sending report emails")
    else:
        logger.warning("No reports generated for any region")
...
"""
    while True:
        ...
        # Show header with current time - Fix: Replace Cyrillic М with Latin M in format string
        now = datetime.now()
