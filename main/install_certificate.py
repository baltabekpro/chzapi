...
def extract_archive(rar_path: str, extract_path: str, password: str = None) -> bool:
    ...
    try:
        ...
        result = subprocess.run(
            cmd, capture_output=True, text=True, timeout=30  # 30 seconds timeout
        )
    ...
    try:
        ...
        if is_pfx:
            ...
            cmd = [
                ...
                "uMy",
                "-pin",
                cert_password,  # Use -pin for password
                "-keep_exportable",
            ]
        else:
            cmd = [certmgr_path, "-inst", "-store", "uMy", "-file", cert_path]

        print("\nInstalling certificate...")
        print(f"Command: {' '.join(cmd)}")  # Debug output
        result = subprocess.run(
            cmd, capture_output=True, text=True, encoding="cp866", timeout=60
        )
        ...
        if result.returncode == 0:
            ...
            if is_pfx and any(
                x in stderr_lower for x in ["пароль", "password", "неверн", "pin"]
            ):
