...
class TelegramBot:
        ...
        try:
            ...
            # Handle certificate selection
                ...
                if cert_name in reports:
                    ...
                    for report in cert_reports:
                        ...
                        if (
                            len(f"report_{report_path}") > 60
                        ):  # Keep within safe limit for callback_data
                            ...
                            keyboard.append(
                                [
                                    InlineKeyboardButton(
                                        f"Отчет от {report_date}",
                                        callback_data=f"report_{report_path}",
                                    )
                                ]
                            )
            ...
            # Handle shortened report selection
                ...
                if (
                    ...
                    and id_hash in self._report_path_map
                ):
    ...
    async def tokens_command(
        ...
        try:
            ...
            keyboard = [
                [InlineKeyboardButton("🔄 Обновить токены", callback_data="refresh")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    async def error_monitoring_task(self) -> None:
        ...
        self.error_queue = (
            asyncio.Queue()
        )  # Initialize the error queue in the event loop
        while self.running:
            try:
                ...
                try:
                    ...
                    error_message = await asyncio.wait_for(
                        self.error_queue.get(), timeout=1.0
                    )
                    ...
                    if "different event loop" in str(e):
                        telegram_logger.error(
                            f"Event loop error in monitoring task: {e}"
                        )
    ...
    async def start_bot(self) -> None:
        ...
        if not self.token or self.token == "YOUR_TELEGRAM_BOT_TOKEN":
            telegram_logger.error(
                "No valid bot token provided. Please update telegram_config.json"
            )
            return
    ...
    def queue_error(self, error_message: str) -> None:
        ...
        try:
            ...
            if (
                ...
                and hasattr(self.application, "loop")
            ):
                try:
                    # Use the application's event loop if available
                    asyncio.run_coroutine_threadsafe(
                        self.error_queue.put(error_message), self.application.loop
                    )
                    telegram_logger.info(
                        f"Error queued for notification: {error_message}"
                    )
                    return
                except Exception as e:
            ...
            try:
                if threading.current_thread() == self.thread:
                    ...
                    telegram_logger.info(
                        f"Error queued for notification using current event loop: {error_message}"
                    )
                    return
            except Exception as e:
            ...
            telegram_logger.info(
                f"Storing error for notification (from external thread): {error_message}"
            )
