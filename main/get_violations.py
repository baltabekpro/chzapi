    ...
    def get_first_violation_date(self, product_group_code: int) -> str:
        ...
        try:
            ...
            response = requests.get(
                ...
                params={"productGroupCode": product_group_code},
            )
            ...
            default_date = (datetime.now() - timedelta(days=365 * 3)).strftime(
                "%Y-%m-%d"
            )
...
def split_date_range(
    start_date: str, end_date: str, product_group_code: int, max_tasks: int = 10
) -> List[tuple[str, str]]:
    """
    ...
    try:
        ...
        while current_start < end:
            ...
            current_end = min(
                current_start + timedelta(days=89), end
            )  # 89 дней + текущий день = 90 дней
...
def save_remaining_dates(
    start_date: str,
    end_date: str,
    product_group_code: int,
    filename: str = "remaining_dates.txt",
) -> None:
    """
            ...
            json.dump(
                {
                    ...
                    "product_group_name": PRODUCT_GROUPS.get(
                        product_group_code, "Неизвестная группа"
                    ),
...
def load_remaining_dates(filename: str = "remaining_dates.txt") -> tuple[str, str, int]:
    ...
    try:
        with open(filename, "r") as f:
            data = json.load(f)
            return (data["start_date"], data["end_date"], data["product_group_code"])
        ...
        if all([saved_start, saved_end, saved_product_code]):
            ...
            if choice == "1":
                ...
                if saved_end_date < today:
                    ...
                    next_start = (saved_end_date + timedelta(days=1)).strftime(
                        "%Y-%m-%d"
                    )
                ...
                while True:
                    try:
                        end_date_input = input(
                            "Введите дату окончания (Enter для текущей даты): "
                        ).strip()
