...
class FileViewer:
    ...
    def list_files(
        self, directory: str = None, filter_ext: List[str] = None
    ) -> List[str]:
        ...
        try:
            ...
            if filter_ext:
                files = [
                    f
                    for f in files
                    if any(f.lower().endswith(ext) for ext in filter_ext)
                ]
    ...
    def show_directory_content(
        self, directory: str = None, page_size: int = 20
    ) -> Optional[str]:
        ...
        try:
            ...
            while True:
                ...
                for i, item in enumerate(
                    all_files[start_idx:end_idx], start=start_idx + 1
                ):
                    item_path = os.path.join(directory, item)
                ...
                if choice.lower() == "q":
                    ...
                    if 1 <= item_num <= total_items:
                        ...
                        input(
                            f"{Fore.RED}Неверный номер файла. Нажмите Enter для продолжения..."
                        )
                else:
    ...
    def view_file(self, filepath: str) -> None:
        ...
        try:
            ...
            print(
                f"{Fore.YELLOW}Последнее изменение: {datetime.fromtimestamp(os.path.getmtime(filepath)).strftime('%Y-%m-%d %H:%M:%S')}"
            )
    ...
    def view_log_file(self, filepath: str, max_lines: int = 500) -> None:
        ...
        try:
            ...
            if len(lines) > max_lines:
                print(
                    f"{Fore.YELLOW}Файл содержит {len(lines)} строк. Показаны последние {max_lines}.\n"
                )
            ...
            for line in lines:
                ...
                if date_match:
                    ...
                    line = line.replace(
                        date_part, f"{Fore.CYAN}{date_part}{Style.RESET_ALL}"
                    )
    ...
    def view_code_file(self, filepath: str, language: str) -> None:
        ...
        try:
            ...
            patterns = {
                ...
                "keyword": re.compile(
                    r"\b(" + "|".join(map(re.escape, lang_keywords)) + r")\b"
                ),
            }
    ...
    def view_text_file(self, filepath: str, max_lines: int = 1000) -> None:
        ...
        try:
            ...
            if len(lines) > max_lines:
                print(
                    f"{Fore.YELLOW}Файл содержит {len(lines)} строк. Показаны первые {max_lines}.\n"
                )
                lines = lines[:max_lines]
    ...
    def view_large_file(
        self, filepath: str, head_lines: int = 20, tail_lines: int = 20
    ) -> None:
        ...
        try:
            ...
            print(
                f"{Fore.YELLOW}Файл содержит {line_count} строк. Показаны первые {head_lines} и последние {tail_lines} строк.\n"
            )
...
def list_files_in_directory(directory=None, detailed=False):
    ...
    try:
        ...
        if detailed:
            # Print header
            print(
                f"{Fore.YELLOW}{'№':4} {'Имя файла':<40} {'Тип':<8} {'Размер':>10} {'Дата изменения':<20}"
            )
            ...
            for i, fname in enumerate(sorted(files), 1):
                ...
                if os.path.isdir(path):
                    print(
                        f"{Fore.GREEN}{i:4d} {fname:<40} {file_type:<8} {size_str:>10} {mod_time:<20}"
                    )
                else:
                    print(
                        f"{Fore.RESET}{i:4d} {fname:<40} {file_type:<8} {size_str:>10} {mod_time:<20}"
                    )
        else:
            # Simple listing
            for i, fname in enumerate(sorted(files), 1):
...
def delete_file(file_path=None):
    ...
    if file_path is None:
        file_path = input(
            f"{Fore.YELLOW}Введите полный путь к файлу для удаления: "
        ).strip()
...
def add_file(src=None, dest_dir=None):
    ...
    if dest_dir is None:
        dest_dir = input(
            f"{Fore.YELLOW}Введите путь к директории для копирования (или Enter для текущей): "
        ).strip()
        if not dest_dir:
            dest_dir = os.getcwd()
...
def check_last_run_info():
    ...
    if not os.path.exists(last_run_file):
        print(
            f"{Fore.YELLOW}Файл last_run.json не найден. Следующий запуск неизвестен."
        )
    ...
    try:
        with open(last_run_file, "r", encoding="utf-8") as f:
            ...
            print(
                f"{Fore.YELLOW}Последний запуск: {last_run.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            print(
                f"{Fore.YELLOW}Следующий запуск: {next_run.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            ...
            if time_until.total_seconds() > 0:
                ...
                print(
                    f"{Fore.GREEN}Времени до следующего запуска: {time_until.days} дней, {hours} часов, {minutes} минут"
                )
            else:
