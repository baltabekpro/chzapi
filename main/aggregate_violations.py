...
def aggregate_violations(json_files: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for violation in data.get("violations", []):
                ...
                violation_type = violation.get(
                    "Вид отклонения", "Неизвестное нарушение"
                )
    ...
    result = {
        ...
        "statistics": {
            product: {
                region: dict(violations) for region, violations in regions.items()
            }
    ...
    result = {
        ...
        "summary": {
            "violations_by_type": dict(
                sorted(violations_by_type.items(), key=lambda x: x[1], reverse=True)
            ),
            "top_products": dict(
                sorted(violations_by_product.items(), key=lambda x: x[1], reverse=True)[
                    :10
                ]
            ),  # Топ-10 товаров
            "top_regions": dict(
                sorted(violations_by_region.items(), key=lambda x: x[1], reverse=True)[
                    :10
                ]
            ),  # Топ-10 регионов
        },
...
def analyze_violations_by_product_and_region(json_files: List[str]) -> Dict[str, Any]:
    ...
    for json_file in json_files:
        try:
            ...
            for violation in data.get("violations", []):
                ...
                product_group = (
                    violation.get("Товарная группа")
                    or violation.get("Группа товаров")
                    or violation.get("Product Group", "")
                )

                region = (
                    violation.get("Регион")
                    or violation.get("Субъект РФ")
                    or violation.get("Субъект")
                    or violation.get("Region", "")
                )

                violation_type = (
                    violation.get("Вид отклонения")
                    or violation.get("Тип нарушения")
                    or violation.get("Violation Type", "")
                )
    ...
    result = {
        "meta": {
            ...
            "unique_regions": len(
                set(region for product in stats.values() for region in product)
            ),
        },
        "overall_statistics": {
            "total_violations": total_violations,
    ...
    for product, regions in stats.items():
        product_total = sum(
            region_stats["total_violations"] for region_stats in regions.values()
        )
...
def main():
    ...
    for product, count in analysis["overall_statistics"]["top_products"].items():
        ...
        for region, stats in product_stats["regions"].items():
            print(
                f"    {region}: {stats['total_violations']} "
