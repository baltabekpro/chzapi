    ...
    try:
        ...
        try:
            ...
            subprocess.check_call(
                [sys.executable, "-m", "pip", "install", "colorama", "requests"]
            )
            print("Basic dependencies installed.")

            # Now create the dependency manager file
            dm_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "dependency_manager.py"
            )
...
from file_utils import check_last_run_info  # Make sure this is imported
from file_utils import add_file, delete_file, get_reports_list, list_files_in_directory
from file_viewer import view_file_with_menu
from get_report import ReportDownloader
from get_tokens import get_tokens
from get_violations import PRODUCT_GROUPS, ViolationsReport
from logger_config import get_logger, log_exception
from process_report import process_reports
from report_processor import process_reports_for_token, view_report
    ...
    for encoding in encodings:
        try:
            with open(file_path, "r", encoding=encoding) as f:
                ...
                reports_logger.info(
                    f"Successfully read file with {encoding} encoding, found {count} violations"
                )
                return count
...
def create_tasks_for_token(cert_name: str, token: str) -> list:
    ...
    violations_logger.info(
        f"Using date range: {yesterday} to {yesterday} (yesterday's data)"
    )

    for group_code in load_product_groups():
        try:
            violations_logger.info(
                f"Creating task for group {group_code} ({PRODUCT_GROUPS.get(group_code, 'Unknown')})"
            )
            ...
            log_exception(
                violations_logger, e, f"Error creating task for group {group_code}"
            )
            continue
        ...
        try:
            ...
            if client.monitor_and_download(task_id, reports_dir):
                ...
                reports_logger.warning(
                    f"Failed to download task {task_id}, will retry later"
                )
    ...
    if remaining_tasks:
        with open(tasks_file, "w") as f:
            for task_id, group_code in remaining_tasks:
                f.write(f"{task_id},{group_code}\n")
        reports_logger.info(
            f"Updated pending tasks, {len(remaining_tasks)} tasks remaining"
        )
    else:
...
def load_email_config():
    ...
    try:
        # Пробуем загрузить конфиг из папки config
        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
        cfg_path = os.path.join(base_dir, 'config', 'email_config.json')
        with open(cfg_path, 'r', encoding='utf-8') as f:
            config = json.load(f)
        email_logger.info(f"Email configuration loaded from {cfg_path}")
        return config
    except Exception as e:
        log_exception(email_logger, e, f"Error loading email config from {cfg_path}")
        return None
    ...
    for csv_file in csv_files:
        try:
            ...
            try:
                ...
                reports_logger.warning(
                    f"Could not extract group code from filename: {csv_file}"
                )
                continue
...
def delete_file():
    ...
    if not os.path.exists(file_path):
        ...
        files_logger.warning(
            f"Attempted to delete non-existent file/directory: {file_path}"
        )
        return
    try:
...
def add_file():
    ...
    if not os.path.isfile(src):
        ...
        return
    dest_dir = input(
        "Введите путь к директории для копирования (или Enter для текущей): "
    ).strip()
    if not dest_dir:
        dest_dir = os.getcwd()
        ...
        try:
            with open(last_run_file, "r", encoding="utf-8") as f:
                ...
                if next_run > now:
                    ...
                    print(
                        f"Времени до следующего запуска: {time_until.days} дней, {hours} часов, {minutes} минут"
                    )
...
"""
    while True:
        ...
        now = datetime.now()
        print(
            f"{Fore.CYAN}╔═══ ЦРПТ Процессор ═══ {now.strftime('%Y-%m-%d %H:%M:%S')} ═══╗"
        )
        ...
        if choice == "1":
            ...
            confirm = input(
                f"{Fore.YELLOW}Вы уверены, что хотите выйти? (д/н): "
            ).lower()
            ...
            json.dump(
                {
                    ...
                    .replace(hour=0, minute=5, second=0)
                    .isoformat(),
                },
                f,
                indent=2,
