...
def load_tokens() -> List[Tuple[str, str]]:
    ...
    try:
        with open("true_api_tokens.json", "r", encoding="utf-8") as f:
            data = json.load(f)
            tokens = [
                (cert_id, token) for cert_id, token in data.get("tokens", {}).items()
            ]
...
def update_token(cert_name, token):
    ...
    try:
        ...
        with open("true_api_tokens.json", "w", encoding="utf-8") as f:
            json.dump(
                {"tokens": tokens, "generated_at": datetime.datetime.now().isoformat()},
                f,
                indent=2,
                ensure_ascii=False,
            )
...
def group_violations_by_region(all_violations):
    ...
    for region in regions_mapping:
        region_violations[region] = {
            "date": next(iter(all_violations.values()))["date"]
            if all_violations
    ...
    empty_regions = [
        r for r in region_violations if not region_violations[r]["tc_data"]
    ]
