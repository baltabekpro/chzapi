...
def add_tc_inn_to_certificate():
    ...
    try:
        ...
        if not is_multi:
            ...
            make_multi = (
                input(
                    f"{Fore.YELLOW}Do you want to enable multiple INNs for this certificate? (y/n): "
                ).lower()
                == "y"
            )
            if make_multi:
        ...
        for pair in existing_pairs:
            if tc in pair:
                update = (
                    input(
                        f"{Fore.YELLOW}ТС {tc} already exists. Update it? (y/n): "
                    ).lower()
                    == "y"
                )
                if update:
                    ...
                    if save_certificate_inns(cert_inns):
                        print(
                            f"{Fore.GREEN}ТС-ИНН pair updated. Now ТС {tc} has ИНН: {inn}"
                        )
                        return True
                    else:
                        return False
        ...
        if save_certificate_inns(cert_inns):
            print(
                f"{Fore.GREEN}ТС-ИНН pair added successfully. Certificate {cert_name} now has ТС {tc} with ИНН {inn}"
            )
            return True
        else:
            return False
...
def add_multiple_tc_inns_to_certificate():
    ...
    try:
        ...
        if new_pairs:
            ...
            if save_certificate_inns(cert_inns):
                print(
                    f"{Fore.GREEN}Added {len(new_pairs)} new TC-INN pairs to certificate {cert_name}"
                )
                return True
            else:
...
def remove_tc_inn_from_certificate():
    ...
    try:
        ...
        pair_choice = int(
            input(f"{Fore.YELLOW}Select ТС-ИНН pair number to remove (0 to cancel): ")
        )
        if pair_choice == 0:
            return False
        pair_choice -= 1
