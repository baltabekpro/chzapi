...
def send_report_emails(reports_data, date_str=None):
    ...
    for region_id, report_data in reports_data.items():
        ...
        if not recipients:
            ...
            logger.info(
                f"No recipients for region {region_name}, using default recipients"
            )

        if not recipients:
            logger.warning(
                f"No recipients configured for region {region_name}, skipping"
            )
            continue

        logger.info(
            f"Sending report for region: {region_id} ({region_name}) to {len(recipients)} recipients"
        )
        total_regions += 1
    ...
    logger.info(
        f"Email sending complete: {success_count}/{total_regions} regions processed successfully"
    )
