...
class EmailTest(unittest.TestCase):
    ...
    def test_single_email(self, mock_smtp):
        ...
        log_result(
            f"  Молочная продукция: {violations_data['violations']['Молочная продукция']} violations"
        )
        log_result(
            f"  Обувные товары: {violations_data['violations']['Обувные товары']} violations"
        )
        log_result(
            f"  Табачная продукция: {violations_data['violations']['Табачная продукция']} violations"
        )
    ...
    def test_regional_emails(self, mock_smtp):
            ...
            email_info = {
                ...
                "region": next(
                    (
                        ...
                        if region_data["name"] in msg["Subject"]
                    ),
                    "Unknown",
                ),
            }
        ...
        log_result(
            f"Regional email sending result: {'SUCCESS' if result else 'FAILED'}"
        )
