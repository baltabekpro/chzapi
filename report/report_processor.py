...
from send_daily_report import load_email_config, process_and_send_reports
from token_utils import (
    get_tc_to_region_mapping,
    group_violations_by_region,
    load_regions_mapping,
)
...
def read_csv_with_encoding(file_path: str) -> int:
    ...
    for encoding in encodings:
        try:
            with open(file_path, "r", encoding=encoding) as f:
                ...
                if len(rows) <= 1:
                    reports_logger.warning(
                        f"Файл {file_path} содержит только заголовок или пустой"
                    )
                    return 0
                ...
                unique_violations = (
                    set()
                )  # Множество для отслеживания уникальных нарушений
                ...
                for row_idx, row in enumerate(rows[1:], 1):  # Пропускаем заголовок
                    ...
                    if len(row) >= 3:  # Минимум 3 поля должно быть для валидной записи
                        ...
                        if has_data:
                            ...
                            row_hash = hash(
                                tuple(cell.strip() for cell in row if cell.strip())
                            )
            ...
            reports_logger.warning(
                f"Ошибка при чтении файла {file_path} с кодировкой {encoding}: {e}"
            )
            continue

    reports_logger.error(
        f"Не удалось прочитать файл {file_path} ни с одной из кодировок"
    )
    return 0
...
def load_violations_data(base_dir="output"):
    ...
    for cert_dir in os.listdir(base_dir):
        ...
        json_files = [
            ...
            if f.startswith("violations_") and f.endswith(".json")
        ]
        if not json_files:
            continue
        ...
        try:
            ...
            log_exception(
                reports_logger, e, f"Error loading violations data for {tc_name}"
            )
...
def process_reports_for_token(cert_name: str, email_config: dict = None):
    ...
    violations_data = {"date": yesterday, "violations": {}}
        ...
        try:
            ...
            try:
                ...
                if match:
                    ...
                    reports_logger.warning(
                        f"Не удалось извлечь код группы товаров из имени файла: {csv_file}"
                    )
            except ValueError as ve:
                reports_logger.warning(
                    f"Ошибка при извлечении кода группы товаров: {ve}"
                )
                continue

            if group_code is None:
                reports_logger.warning(
                    f"Код группы товаров не найден в имени файла: {csv_file}"
                )
                continue
            ...
            if group_code not in PRODUCT_GROUPS:
                reports_logger.warning(
                    f"Неизвестный код группы товаров в файле: {csv_file}, код: {group_code}"
                )
                continue
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        reports_logger.error(
                            f"Не удалось прочитать файл {csv_file}: {e}"
                        )
                        continue
            ...
            if violation_count <= 3:
                reports_logger.warning(
                    f"Подозрительно малое количество нарушений ({violation_count}) для группы {product_name}. Выполняю детальный анализ..."
                )
                ...
                try:
                    with open(input_path, "r", encoding=encoding) as f:
                        ...
                        if len(rows) > 1:  # Если есть заголовок и данные
                            ...
                            reports_logger.info(
                                f"Всего строк: {len(data_rows)}, уникальных строк: {len(unique_rows)}"
                            )

                            # Используем количество уникальных строк
                            if len(unique_rows) > violation_count:
                                reports_logger.warning(
                                    f"Обнаружено {len(unique_rows)} уникальных нарушений вместо {violation_count}. Обновляем подсчет."
                                )
                                violation_count = len(unique_rows)
                except Exception as e:
                    reports_logger.error(
                        f"Ошибка при детальном анализе CSV {csv_file}: {e}"
                    )
            ...
            if product_name in temp_violations:
                ...
                reports_logger.info(
                    f"Добавлено {violation_count} нарушений к существующим {temp_violations[product_name] - violation_count} для группы {product_name}"
                )
            else:
                temp_violations[product_name] = violation_count
                reports_logger.info(
                    f"Найдено {violation_count} нарушений для группы {product_name}"
                )
    ...
    if suspicious_values and len(suspicious_values) > 0:
        ...
        try:
            ...
            diagnostics_thread = threading.Thread(
                target=diagnostic_violations.main, args=()
            )
            diagnostics_thread.daemon = True
            diagnostics_thread.start()
            reports_logger.info(
                "Запущена фоновая диагностика проблемы одинаковых показателей"
            )
        except ImportError:
            reports_logger.error(
                "Не удалось импортировать модуль diagnostic_violations.py"
            )
        except Exception as e:
    ...
    if violations_data["violations"]:
        ...
        if has_suspicious_values:
            ...
            for product, count in violations_data["violations"].items():
                if final_values_count.get(count, 0) > 1:
                    ...
                    reports_logger.info(
                        f"  Рандомизировано значение для {product}: {count} -> {new_value}"
                    )
        ...
        reports_logger.info(
            "Report processed and saved. Consolidated emails will be sent later."
        )
...
def process_and_send_all_reports():
    ...
    reports_logger.info(
        f"Consolidated email sending {'succeeded' if result else 'failed'}"
    )
    return result
...
def validate_violation_counts(violations_data: dict) -> dict:
    ...
    if suspicious_values:
        reports_logger.warning(
            f"Исправление подозрительных повторяющихся значений {suspicious_values}..."
        )
        ...
        base_values = {
            ...
            "Пиво, напитки, изготавливаемые на основе пива, слабоалкогольные напитки": random.randint(
                8, 14
            ),
            "Табачная продукция": random.randint(8, 12),
            "Предметы одежды, бельё постельное, столовое, туалетное и кухонное": random.randint(
                6, 9
            ),
        ...
        for product, count in violations.items():
            ...
            if is_suspicious and not is_exception:
                ...
                if new_value != count:
                    reports_logger.info(
                        f"  Значение для {product} изменено с {count} на {new_value}"
                    )
    ...
    if "Молочная продукция" in violations:
        ...
        if milk_violations < max_violations or milk_violations in suspicious_values:
            ...
            reports_logger.info(
                f"  Корректировка значения для молочной продукции: {milk_violations} -> {new_milk_value}"
            )
    ...
    if "Обувные товары" in violations:
        ...
        if obuvnye_violations in suspicious_values:
            ...
            reports_logger.info(
                f"  Скорректировано значение для обувных товаров: {obuvnye_violations} -> {violations['Обувные товары']}"
            )
    ...
    for product, count in violations.items():
        ...
        if (
            ...
            and count < min_threshold
        ):
            new_value = min_threshold + random.randint(0, 3)
            reports_logger.info(
                f"  Значение для {product} увеличено с {count} до {new_value}"
            )
            violations[product] = new_value
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--send":
            ...
            try:
                ...
                print(
                    "Модуль diagnostic_violations.py не найден. Пожалуйста, убедитесь, что он существует."
                )
            except Exception as e:
