...
def test_ts_to_region_mapping():
    ...
    # Load regions data
    regions_data = region_manager.load_regions_data()
    ...
    print(f"{Fore.CYAN}{'TC':<40} | {'Region':<20} | {'Status'}")
    print(f"{Fore.CYAN}{'-'*75}")

    for tc in all_tcs:
        region_id = region_manager.get_tc_region(tc)
        if region_id != "Undefined":
            region_name = regions_data[region_id].get("name", "Unknown")
            print(f"{Fore.GREEN}{tc:<40} | {region_name:<20} | ✓ Assigned")
            tcs_with_region += 1
        else:
            print(f"{Fore.YELLOW}{tc:<40} | {'None':<20} | ⚠ Not assigned")
    ...
    if tcs_without_region > 0:
        print(
            f"{Fore.YELLOW}Assign the unassigned TCs to regions to ensure proper report grouping."
        )
    else:
