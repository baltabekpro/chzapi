...
try:
    ...
    from email_utils import load_email_config, send_test_email, send_violations_report
    from file_utils import check_last_run_info, list_files_in_directory

    from main import (
        generate_report_for_region,
        load_product_groups,
        load_tokens,
        process_reports,
        process_reports_for_token,
    )
    from main import send_violations_report as main_send_violations
except ImportError as e:
    print(f"{Fore.RED}Error importing modules: {e}")
    print(
        f"{Fore.YELLOW}Make sure you're running this test from the project root directory."
    )
    sys.exit(1)
...
class TestReportSystem(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with open(TEST_RESULTS_FILE, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"=== Test run started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===\n"
            )
            f.write(f"====================================================\n\n")
    ...
    def tearDownClass(cls):
        ...
        with open(TEST_RESULTS_FILE, "a", encoding="utf-8") as f:
            f.write(f"\n====================================================\n")
            f.write(
                f"=== Test run completed at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===\n"
            )
            f.write(f"====================================================\n")
    ...
    def create_mock_data(cls):
        ...
        os.makedirs(
            os.path.join(cls.test_dir, "output/Хуторская Татьяна - тс5"), exist_ok=True
        )
        os.makedirs(
            os.path.join(cls.test_dir, "output/Лежнева Евгения - тс54"), exist_ok=True
        )
        ...
        violations_ts5 = {
            "date": yesterday,
            "violations": {"Молочная продукция": 10, "Обувные товары": 5},
        }

        # Mock violations for ts54
        violations_ts54 = {
            "date": yesterday,
            "violations": {"Молочная продукция": 7, "Табачная продукция": 15},
        }
        ...
        with open(
            os.path.join(
                cls.test_dir,
                f"output/Хуторская Татьяна - тс5/violations_{yesterday}.json",
            ),
            "w",
            encoding="utf-8",
        ) as f:
        ...
        with open(
            os.path.join(
                cls.test_dir,
                f"output/Лежнева Евгения - тс54/violations_{yesterday}.json",
            ),
            "w",
            encoding="utf-8",
        ) as f:
        ...
        with open(
            os.path.join(cls.test_dir, "regions.json"), "w", encoding="utf-8"
        ) as f:
        ...
        with open(
            os.path.join(cls.test_dir, "email_config.json"), "w", encoding="utf-8"
        ) as f:
        ...
        with open(
            os.path.join(cls.test_dir, "cert_inns.json"), "w", encoding="utf-8"
        ) as f:
            json.dump(cert_inns, f, ensure_ascii=False, indent=2)

        # Create mock products.txt
        with open(
            os.path.join(cls.test_dir, "products.txt"), "w", encoding="utf-8"
        ) as f:
            f.write("10\n")  # молочная продукция
            f.write("3\n")  # табачная продукция
            f.write("2\n")  # обувь
    ...
    def test_email_sending(self, mock_smtp):
        ...
        log_test_result(
            f"Successfully loaded email configuration: {email_config['smtp_server']}"
        )
        ...
        log_test_result(
            f"Violations data: {json.dumps(violations_data, ensure_ascii=False)}"
        )
    ...
    def test_report_processing(self):
        ...
        log_test_result(
            f"Testing report generation for region: {region_info['name']} ({region_id})"
        )
        log_test_result(
            f"Region configuration: {json.dumps(region_info, ensure_ascii=False)}"
        )
    ...
    def test_email_report_sending(self, mock_smtp):
        ...
        log_test_result(
            f"Prepared reports data for regions: {', '.join(reports_data.keys())}"
        )
        for region_id, report in reports_data.items():
            log_test_result(
                f"Region {region_id} report: {json.dumps(report, ensure_ascii=False)}"
            )
        ...
        def capture_message(msg):
            email_data = {
                ...
                "to": msg["To"],
                "from": msg["From"],
                "region": msg["Subject"].split(" - ")[1]
                if " - " in msg["Subject"]
                else "Unknown",
            }
            sent_emails.append(email_data)
            log_test_result(
                f"Would send email: {json.dumps(email_data, ensure_ascii=False)}"
            )
            ...
            for part in msg.get_payload():
                if part.get_content_type() == "text/html":
                    content = part.get_payload()
                    log_test_result(
                        f"Email content for {email_data['region']} (first 10 lines):"
                    )
                    lines = content.split("\n")
    ...
    def test_file_operations(self):
        ...
        with mock.patch("builtins.open", mock.mock_open(read_data="10\n3\n2\n")):
            ...
            log_test_result(
                f"Successfully loaded {len(groups)} product groups: {groups}"
            )
    ...
    def test_region_management(self):
        ...
        with mock.patch(
            "region_manager.load_regions_data", return_value={}
            ...
            with mock.patch(
                "region_manager.load_regions_data",
                return_value={
                    region_id: {"name": region_name, "emails": emails, "tc_list": []}
                },
            ):
                tc_name = "Test TC"
                ...
                self.assertTrue(success)
                log_test_result(
                    f"Successfully added TC {tc_name} to region {region_name}"
                )
                ...
                with mock.patch(
                    ...
                    log_test_result(
                        f"Successfully found TC {tc_name} in region {region_id}"
                    )

                    # Test TC that doesn't exist
                    non_existent_tc = "Non-existent TC"
                    log_test_result(
                        f"Testing finding region for non-existent TC {non_existent_tc}"
                    )

                    found_region = region_manager.get_tc_region(non_existent_tc)
                    self.assertEqual(found_region, "Undefined")
                    log_test_result(
                        f"TC {non_existent_tc} correctly returned region {found_region} (expected: Undefined)"
                    )
    ...
    def test_process_reports_for_token(self, mock_smtp):
        ...
        self.assertTrue(
            os.path.exists(report_file), f"Report file {report_file} was not created"
        )
        ...
        try:
            with open(report_file, "r", encoding="utf-8") as f:
                ...
                log_test_result(
                    f"Successfully read report file content: {json.dumps(data, ensure_ascii=False)}"
                )
    ...
    def test_complete_process(self):
        ...
        with mock.patch(
            "main.get_tokens",
            return_value=[("Certificate 1", "token1"), ("Certificate 2", "token2")],
        ):
            log_test_result("Mocked get_tokens to return 2 test certificates")

            # Mock create_tasks_for_token
            with mock.patch(
                "main.create_tasks_for_token",
                return_value=[("task1", 10), ("task2", 3)],
            ):
                ...
                with mock.patch("main.download_tasks_for_token") as mock_download:
                    ...
                    with mock.patch("main.process_reports_for_token") as mock_process:
                        ...
                        with mock.patch(
                            "send_daily_report.process_and_send_reports",
                            return_value=True,
                        ) as mock_send:
                            log_test_result(
                                "Mocked send_daily_report.process_and_send_reports"
                            )
                            ...
                            with mock.patch("main.run_daily_process") as mock_run:
                                ...
                                log_test_result(
                                    "Mocked main.run_daily_process to return True"
                                )
                                ...
                                log_test_result(
                                    "1. Refresh tokens - calls get_tokens() to fetch new tokens for all certificates"
                                )
                                log_test_result(
                                    "   - This creates authentication tokens for API access"
                                )

                                log_test_result(
                                    "2. Create tasks for each product group - for each certificate and product group:"
                                )
                                log_test_result(
                                    "   - Creates API tasks for retrieving violation data"
                                )
                                log_test_result(
                                    "   - Each task is for a specific product group (e.g., dairy, tobacco)"
                                )
                                log_test_result(
                                    "   - Tasks are created with yesterday's date range"
                                )

                                log_test_result(
                                    "3. Download task results - for each certificate:"
                                )
                                log_test_result(
                                    "   - Monitors task status and downloads results when completed"
                                )
                                log_test_result(
                                    "   - Results are stored as CSV files in certificate's report directory"
                                )

                                log_test_result(
                                    "4. Process reports - for each certificate:"
                                )
                                log_test_result(
                                    "   - Parses downloaded CSV files to count violations by product group"
                                )
                                log_test_result(
                                    "   - Generates consolidated JSON report with all violations"
                                )
                                log_test_result(
                                    "   - Links each trading company to its region"
                                )

                                log_test_result(
                                    "5. Send consolidated reports by region:"
                                )
                                log_test_result(
                                    "   - Groups all violation data by region"
                                )
                                log_test_result(
                                    "   - Generates HTML email report for each region"
                                )
                                log_test_result(
                                    "   - Sends reports to region-specific email recipients"
                                )
