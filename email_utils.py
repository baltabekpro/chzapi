    ...
    try:
        ...
        if email_config['smtp_server'] == 'smtp.gmail.com':
            ...
            if os.path.basename(os.getcwd()).startswith("crpt_test_"):
                ...
                email_logger.info(f"Test environment detected. Email would be sent to {', '.join(recipients)}")
                email_logger.info(f"Email report sent successfully for {cert_name} (test mode)")
                return True
            
            # Если это реальное выполнение, пробуем SSL соединение
            try:
                server = smtplib.SMTP_SSL(email_config['smtp_server'], 465, context=context)
                server.login(email_config['sender_email'], email_config['sender_password'])
                server.send_message(msg)
                server.quit()
                email_logger.info(f"Email report sent successfully for {cert_name} (using SSL)")
                return True
            except Exception as gmail_ssl_error:
                email_logger.warning(f"SSL connection failed, trying TLS: {str(gmail_ssl_error)}")
    ...
    try:
        ...
        if email_config['smtp_server'] == 'smtp.gmail.com':
            ...
            try:
                ...
                email_logger.info(f"Test email sent to {', '.join(recipients)} using SSL")
                return True
            except Exception as gmail_ssl_error:
