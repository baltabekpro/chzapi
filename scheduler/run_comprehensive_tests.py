...
def create_mock_data(temp_dir):
    ...
    violations_data = {
        "Иванов - тс1": {
            "date": yesterday,
            "violations": {"Молочная продукция": 8, "Обувные товары": 3},
        },
        "Петров - тс2": {
            "date": yesterday,
            "violations": {"Молочная продукция": 5, "Табачная продукция": 12},
        },
        "Сидоров - тс3": {
            "date": yesterday,
            "violations": {"Табачная продукция": 7, "Обувные товары": 9},
        },
    }
    ...
    for cert, data in violations_data.items():
        report_file = os.path.join(
            temp_dir, "output", cert, f"violations_{yesterday}.json"
        )
    ...
    for cert in certs:
        ...
        csv_file = os.path.join(
            report_dir, f"violations_group10__{yesterday.replace('-', '')}_120000.csv"
        )
...
class ComprehensiveTests:
    ...
    def run_all_tests(self):
        ...
        log_message(
            ...
            "SUCCESS" if self.tests_passed > 0 else "INFO",
        )
        log_message(
            f"Tests failed: {self.tests_failed}",
            "ERROR" if self.tests_failed > 0 else "SUCCESS",
        )
    ...
    def test_file_operations(self):
        ...
        try:
            ...
            if set(certs) == set(expected_certs):
                ...
                log_message(
                    f"Failed to list certificate directories. Expected: {expected_certs}, Got: {certs}",
                    "ERROR",
                )
                self.tests_failed += 1
            ...
            yesterday = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")
            report_file = os.path.join(
                output_dir, "Иванов - тс1", f"violations_{yesterday}.json"
            )
            ...
            with open(report_file, "r", encoding="utf-8") as f:
                ...
                if data["date"] == yesterday and "violations" in data:
                    ...
                    log_message(
                        "Failed to read proper JSON data from report file", "ERROR"
                    )
                    self.tests_failed += 1
    ...
    def test_region_management(self):
        ...
        try:
            ...
            if len(regions) == 3:
                ...
                log_message(
                    f"Failed to load regions. Expected 3 regions, got {len(regions)}",
                    "ERROR",
                )
                self.tests_failed += 1
            ...
            if region_id == "region1":
                log_message(
                    f"Successfully found region for TC {tc_name}: {region_id}",
                    "SUCCESS",
                )
                self.tests_passed += 1
            else:
                log_message(
                    f"Failed to find correct region for TC {tc_name}. Expected 'region1', got '{region_id}'",
                    "ERROR",
                )
                self.tests_failed += 1
            ...
            with mock.patch("region_manager.save_regions_data", return_value=True):
                success = region_manager.add_region(
                    new_region_id, new_region_name, emails
                )
                if success:
                    log_message(
                        f"Successfully added new region: {new_region_name}", "SUCCESS"
                    )
                    self.tests_passed += 1
                else:
    ...
    def test_email_functionality(self, mock_basename, mock_smtp_ssl, mock_smtp):
        ...
        try:
            ...
            def capture_email(msg):
                ...
                log_message(
                    f"Email would be sent: {json.dumps(email_info, ensure_ascii=False)}"
                )
            ...
            report_file = os.path.join(
                self.temp_dir, "output", cert_name, f"violations_{yesterday}.json"
            )
            with open(report_file, "r", encoding="utf-8") as f:
            ...
            if result:
                log_message(
                    f"Successfully sent violations report for {cert_name}", "SUCCESS"
                )
                self.tests_passed += 1
            else:
                log_message(
                    f"Failed to send violations report for {cert_name}", "ERROR"
                )
                self.tests_failed += 1
            ...
            if len(emails_sent) > 0:
                log_message(
                    f"Email was correctly generated and would be sent", "SUCCESS"
                )
                self.tests_passed += 1
            else:
                ...
                if mock_smtp_ssl.called or mock_smtp.called:
                    log_message(
                        f"SMTP connection was attempted, this is acceptable", "SUCCESS"
                    )
                    self.tests_passed += 1
                else:
    ...
    def test_report_processing(self):
        ...
        try:
            ...
            if report:
                log_message(
                    f"Successfully generated report for region {region_info['name']}",
                    "SUCCESS",
                )
                log_message(
                    f"Report data: {json.dumps(report, ensure_ascii=False, indent=2)}"
                )
                self.tests_passed += 1
            else:
                log_message(
                    f"Failed to generate report for region {region_info['name']}",
                    "ERROR",
                )
                self.tests_failed += 1
            ...
            if "violations" in report and "total_violations" in report:
                ...
                if total == report["total_violations"]:
                    log_message(
                        f"Report content is valid. Total violations: {total}", "SUCCESS"
                    )
                    self.tests_passed += 1
                else:
                    log_message(
                        f"Report totals don't match. Sum: {total}, Reported: {report['total_violations']}",
                        "ERROR",
                    )
                    self.tests_failed += 1
            else:
        ...
        try:
            ...
            with mock.patch("main.read_csv_with_encoding", return_value=5):
                for cert in certificates:
                    ...
                    csv_file = os.path.join(
                        reports_dir,
                        f"violations_group10__{yesterday.replace('-', '')}_120000.csv",
                    )
                    ...
                    output_file = os.path.join(
                        "output", cert, f"violations_{yesterday}.json"
                    )
                    if os.path.exists(output_file):
                        log_message(
                            f"Successfully processed reports for {cert}", "SUCCESS"
                        )
                        self.tests_passed += 1
                    else:
            ...
            with mock.patch(
                "send_daily_report.generate_consolidated_report_by_region",
                return_value={
                    "region1": {
                        "region_name": "Москва",
                        "date": yesterday,
                ...
                if result:
                    log_message(
                        "Successfully sent consolidated regional reports", "SUCCESS"
                    )
                    self.tests_passed += 1
                else:
                ...
                log_message(
                    "Verified that regional report generation and sending flow works",
                    "SUCCESS",
                )
                self.tests_passed += 1
