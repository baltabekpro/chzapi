...
from typing import Any, Dict, List, Optional  # Add Optional import
...
def get_event_loop_for_thread(thread_id=None):
    """Get the event loop for a specific thread, creating one if necessary"""
    if thread_id is None:
        thread_id = threading.get_ident()

    global _event_loops, _loop_locks

    # Create lock for this thread if it doesn't exist
    if thread_id not in _loop_locks:
        _loop_locks[thread_id] = threading.Lock()

    with _loop_locks[thread_id]:
...
class Scheduler:
    ...
    def load_config(self):
        ...
        try:
            if os.path.exists(self.config_file):
                ...
                scheduler_logger.info(
                    f"Created default configuration in {self.config_file}"
                )
            ...
            try:
                with open("email_config.json", "r", encoding="utf-8") as f:
                    ...
                    if "send_time" in email_config:
                        ...
                        scheduler_logger.info(
                            f"Using email time from email_config.json: {self.config['email_time']}"
                        )
            except Exception as e:
                scheduler_logger.warning(
                    f"Couldn't read email time from email_config.json: {e}"
                )
            ...
            if result:
                ...
                try:
                    ...
                    send_telegram_notification(
                        "✅ Ежедневная обработка успешно завершена"
                    )
                except ImportError:
                    pass
            else:
        ...
        try:
            ...
            if result:
                ...
                try:
                    ...
                    send_telegram_notification(
                        "✅ Отчеты по электронной почте успешно отправлены"
                    )
                except ImportError:
                    pass
            else:
                ...
                try:
                    ...
                    send_telegram_notification(
                        "❌ Не удалось отправить отчеты по электронной почте"
                    )
                except ImportError:
                    pass
            ...
            try:
                ...
                send_telegram_notification(
                    f"❌ Ошибка при отправке отчетов по электронной почте: {str(e)}"
                )
            except ImportError:
                pass
    ...
    def update_last_run_time(self):
        ...
        try:
            last_run = {
                ...
                "scheduler_run": True,
                "next_scheduled_run": (datetime.now() + timedelta(days=1))
                .replace(
    ...
    def update_last_email_run_time(self):
        ...
        try:
            last_run = {"last_run": datetime.now().isoformat(), "manual_run": False}
    ...
    def check_and_run_tasks(self):
        ...
        if self.is_time_to_run(self.config["daily_report_time"]):
            scheduler_logger.info(
                f"It's time to run daily report ({self.config['daily_report_time']})"
            )
        ...
        if self.is_time_to_run(self.config["token_refresh_time"]):
            scheduler_logger.info(
                f"It's time to refresh tokens ({self.config['token_refresh_time']})"
            )
            self.refresh_tokens()
        ...
        if self.is_time_to_run(self.config["email_time"]):
            scheduler_logger.info(
                f"It's time to send email reports ({self.config['email_time']})"
            )
    ...
    def run_continuously(self):
        ...
        def custom_exception_handler(loop, context):
            ...
            if exception:
                error_msg = f"Caught exception: {exception}"
            else:
                error_msg = f"Error: {context.get('message', 'Unknown error')}"

            scheduler_logger.error(error_msg)
            # Create a new task to report the error, avoiding Queue issues
            asyncio.create_task(self.report_error(error_msg))

        loop.set_exception_handler(custom_exception_handler)

        # Use the event loop for main scheduler loop
        try:
            while self.running:
            ...
            log_exception(
                scheduler_logger, e, "Error in scheduler continuous operation"
            )
            self.running = False
    ...
    async def report_error(self, error_message):
        ...
        try:
            # Import from our new helper module
            from telegram_handlers import send_notification

            send_notification(f"❌ Scheduler error: {error_message}")
        except Exception as e:
            scheduler_logger.error(f"Failed to report error: {e}")
    ...
    def run_test(self, delay_minutes=2):
        """Run test process after specified delay"""
        scheduler_logger.info(
            f"Starting test scheduler with {delay_minutes} minute delay"
        )

        # Schedule test run
        target_time = datetime.now() + timedelta(minutes=int(delay_minutes))
        scheduler_logger.info(
            f"Test process scheduled for {target_time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        ...
        try:
            ...
            while datetime.now() < target_time:
                ...
                if remaining > 5:  # Only log if more than 5 seconds remain
                    scheduler_logger.info(
                        f"Waiting for test process: {int(remaining)} seconds remaining"
                    )
                    time.sleep(
                        min(remaining, 30)
                    )  # Sleep for up to 30 seconds at a time
                else:
                    time.sleep(remaining)  # Sleep for the remaining time

            # Run the test process
            scheduler_logger.info("Executing test process")

            # Import and run the test process
            from main import run_test_process

            result = run_test_process()

            if result:
                scheduler_logger.info("Test process completed successfully")
            else:
                scheduler_logger.error("Test process failed")

        except Exception as e:
            log_exception(scheduler_logger, e, "Error in test process")

        scheduler_logger.info("Test scheduler finished")
                ...
                try:
                    ...
                    if "python" in proc.info["name"].lower():
                        ...
                        if proc.info["cmdline"] and len(proc.info["cmdline"]) > 1:
                            ...
                            if (
                                "--scheduler" in cmdline
                                and os.path.basename(__file__) in cmdline
                            ):
        ...
        if os.name == "nt":
            ...
            result = subprocess.run(
                [
                    "tasklist",
                    "/FI",
                    f"WINDOWTITLE eq *{os.path.basename(__file__)}*",
                    "/FO",
                    "CSV",
                ],
                capture_output=True,
...
def start_test_scheduler(delay_minutes=2):
    """
    Start a test scheduler that will run once after the specified delay

    Args:
        delay_minutes: Number of minutes to wait before running test

    Returns:
        int or None: PID of the test scheduler process if running, None otherwise
    """
    scheduler_logger.info(f"Starting test scheduler with {delay_minutes} minute delay")

    try:
        # Get Python executable and current script path
        python_exe = sys.executable
        script_path = os.path.abspath(__file__)

        # Create a detached process with test parameters
        if os.name == "nt":  # Windows
            import subprocess
            ...
            process = subprocess.Popen(
                [python_exe, script_path, "--test", str(delay_minutes)],
                close_fds=True,
                creationflags=subprocess.CREATE_NEW_PROCESS_GROUP
                | subprocess.DETACHED_PROCESS,
                startupinfo=startupinfo,
        ...
        pid = process.pid
        scheduler_logger.info(f"Started test scheduler with PID {pid}")
        return pid
    except Exception as e:
        scheduler_logger.error(f"Error starting test scheduler: {e}")
        return None
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--test":
        ...
        scheduler_logger.info(
            f"Running scheduler in test mode with {delay_minutes} minute delay"
        )
        scheduler = Scheduler()
        scheduler.run_test(delay_minutes)
    elif len(sys.argv) > 1 and sys.argv[1] == "--scheduler":
        # Normal scheduler mode
        scheduler_logger.info("Running scheduler in normal mode")
        scheduler = Scheduler()
        scheduler.run_continuously()
    else:
        # Interactive mode - just exit
        scheduler_logger.info("Scheduler module loaded in interactive mode")
